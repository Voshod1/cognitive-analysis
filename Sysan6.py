...
class SelfDrivingCarMap:
    ...
    def form_connections(self):
        # self.connections = pd.read_csv("out.csv").set_index("Unnamed: 0")
        self.connections = pd.read_excel("data_raw_1.xlsx").set_index("Unnamed: 0")
        print("hello")
...
class UI(QDialog):
    def __init__(self):
        ...
        self.switch_color_mode_button = QCheckBox(" ")
        self.switch_color_mode_button.setChecked(True)

        self.plot_scenario_dynamics_button = QPushButton("Графік динаміки сценарію")
        self.plot_scenario_dynamics_button.setFlat(True)

        self.send_impulse_button = QPushButton("Послати імпульс")
        self.send_impulse_button.setFlat(True)

        self.load_cognitive_map_button = QPushButton("Завантажити когнітивну карту")
        self.load_cognitive_map_button.setFlat(True)

        self.generate_random_graph_button = QPushButton("Випадковий граф")
        self.generate_random_graph_button.setFlat(True)

        self.reset_outputs_button = QPushButton("Скинути")
    ...
    def check_structural_stability(self):
        try:
            ...
            cycles = reduce(lambda a, b: a + b, ["Парність: " + str(np.round(np.array(cycle[0]), 2)) + "  |  Цикл: "
            ...
            self.structural_stability_value.setText("Нема циклів")

    def check_numerical_stability(self):
        stability = "Стабільна" if self.graph.stability() else "Нестабільна"
    ...
    def plot_scenario_dynamics(self):
        ...
        dialog.setWindowIcon(QIcon("icon.jpg"))
        dialog.setWindowTitle("Динаміка сценарію")
        dialog.setWindowIconText("Динаміка сценарію")
        ...
        dialog.text_box = QTextEdit("")
        dialog.text_box.setPlaceholderText("Список доданих вершин")

        dialog.append_vertex_button = QPushButton("Додати вершину")
